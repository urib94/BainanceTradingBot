...
public class MACDOverRSILongExitStrategy3 extends MACDOverRSIBaseExitStrategy {
	...
	public SellingInstructions run(RealTimeData realTimeData, boolean isTrailing) {
		...
		} else if (realTimeData.getMacdOverRsiValueAtIndex(realTimeData.getLastIndex()) > Config.ZERO && decliningPyramid(realTimeData, DecliningType.POSITIVE)){
			TelegramMessenger.sendToTelegram("exiting position with long exit 3" + "time: " + new Date(System.currentTimeMillis()));
