...
public class RSIExitStrategy2 implements ExitStrategy {
	...
	/**
	 * Checks if the current close of RSIIndicator value is rsiAbove 73, and then below 70 and then below 60.
	 * @param realTimeData
	...
	public BigDecimal run(RealTimeData realTimeData) {
		if (positionInStrategy == PositionInStrategy.POSITION_ONE) {
			if (realTimeData.rsiAbove(RealTimeData.RSIType.CLOSE, RSIConstants.RSI_EXIT_OPTION_2_OVER_THRESHOLD1)) {
			...
			if (! realTimeData.rsiAbove(RealTimeData.RSIType.CLOSE, RSIConstants.RSI_EXIT_OPTION_2_UNDER_THRESHOLD1)) {
			...
			if (! realTimeData.rsiAbove(RealTimeData.RSIType.CLOSE, RSIConstants.RSI_EXIT_OPTION_2_UNDER_THRESHOLD2)) {
