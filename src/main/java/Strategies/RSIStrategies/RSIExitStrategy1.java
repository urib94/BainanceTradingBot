...
public class RSIExitStrategy1 implements ExitStrategy {
	...
	/**
	 * Returns if the rsi Indicator went rsiAbove 73 and then crossed down 60, and then to 50.
	 * @param realTimeData
	...
	public BigDecimal run(RealTimeData realTimeData) {
		if (positionInStrategy == PositionInStrategy.POSITION_ONE) {
			if (realTimeData.rsiAbove(RealTimeData.RSIType.CLOSE, RSIConstants.RSI_EXIT_OPTION_1_OVER_THRESHOLD1)
					&& !(realTimeData.rsiAbove(RealTimeData.RSIType.CLOSE, RSIConstants.RSI_EXIT_OPTION_2_OVER_THRESHOLD1))) {
			...
			if ( ! realTimeData.rsiAbove(RealTimeData.RSIType.CLOSE, RSIConstants.RSI_EXIT_OPTION_1_UNDER_THRESHOLD1)) {
			...
			if (! realTimeData.rsiAbove(RealTimeData.RSIType.CLOSE, RSIConstants.RSI_EXIT_OPTION_1_UNDER_THRESHOLD2)) {
